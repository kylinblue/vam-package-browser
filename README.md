...
A local Windows desktop app that turns a sprawling Virt-A-Mate `.var` library
into a fast, gallery-style visual browser. Tauri 2.x + React/TS frontend,
Rust backend, SQLite index, on-disk thumbnail cache. The user's library
(default `D:\Games\VAM\AddonPackages`) is **strictly read-only**; everything
the app derives lives under `%APPDATA%\com.github.kylinblue.vam-package-browser\`.

The pitch: scan once, classify aggressively, then browse like a media
gallery. Heavy work (archive walking, meta parsing, thumbnail generation,
LLM tagging, hub matching, embedding) is done up front and cached, so the
hot path is just SQLite reads plus pre-generated WebP thumbs.
...
PowerShell on this machine has quirks; see [CLAUDE.md](CLAUDE.md) for the
full set. Short version:

```powershell
& npm.cmd install                                                  # frontend deps
...
`scripts\dev-env.cmd` sources the MSVC `vcvars64.bat` and prepends cargo to
PATH so the linker and compiler resolve. Use the prebuilt debug binaries
under `src-tauri\target\debug\` to invoke CLI tools directly — `cargo run
--release --bin foo` triggers a multi-minute LTO recompile for no benefit.
...
`D:\Games\VAM\AddonPackages` is opened with read-only file handles only.
The scanner never writes back. All derived state (`index.sqlite`,
`thumbs/`) lives under `%APPDATA%\com.github.kylinblue.vam-package-browser\`. The
future "visibility presets" milestone will create/move/symlink files, but
only into a *separate* active folder, with dry-run preview and explicit
user confirmation — never into the master library.
